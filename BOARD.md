...
最終更新: 2026-09-05（第2回会議）。これが「現行の決定事項」の唯一の台帳。会議録は `docs/board/YYYY-MM-DD.md`、各担当の詳細は `docs/board/2026-09-05-{monetization,taxonomy,content,design}.md`。
...
- 月1万円の数式（12月、アクティブ ≤100人前提、**由来調査の販売は含めない**）: 会員 500円×12人（月10 + 年2の月割）≈ 5,800 ／ 取扱店掲載 1,000円×3店 = 3,000 ／ アフィリ ≈ 500 → **約9,300〜10,000円**。不足分は年額会員の前払い（5,000円/人）で現金ベースでは埋まる。11月末レビューで会員5人未満なら 12月目標を 1月末へ後ろ倒し（価格は動かさない）。
- 原価: AI 調査 ≤1,500円/月、Stripe 手数料 ≈ 350円/月。
- 現状（2026-09-05）: 売上 ¥0、登録 6人（3月以降新規ゼロ）、有料 0人、公開 Anthurium 31件（原種21・Hybrid 3・Clone 5・実生2）、非公開 Monstera 11・Philodendron 9・Platycerium 18、写真 40枚/28品種、投票・お気に入りの利用 0件、Stripe 審査中。
...
| アフィリエイト | 楽天・Yahoo!(VC) のみ、**Amazon リンク削除**。掲載は **産地ページ + 自生地環境（標高・雲霧林・着生）を持つ原種ページのみ**。トップ・属ページ・Hybrid/Clone/実生ページには置かない。見出し直下に「PR」常時。期待 +500円/月 | 決定 09-05 |
...
- **Stripe を先に**（09-05 提出、決済一時停止中）。2〜3日でステータスが変わらなければサポート問い合わせ（`docs/stripe-application.md` §5）。承認 → 会員 Price 2本作成 → 鍵受け渡し → 1プラン化デプロイ。不承認 → `_PAYMENT_PROVIDER=payjp`、PAY.JP 申請（プランは会員2本のみ）。判定後、敗者側の関数・分岐は削除。
...
既定回答（オーナー未確認・異論なければ確定）: Glorious / Splendid / Majestic → Hybrid ／ 'Dark Mama' → disputed 両説 ／ 'Albo Variegata' 'Aurea' → Clone + tag `variegata`+`line` ／ 未収録ページは noindex ／ 'Michelle' 'Zara' は Hybrid。オーナー回答済: 'King of Spades' Clone、'Angels dream' 綴り維持。

データ追加修正（09-05、[即]）: 'Red Crystallinum'(52) `formula_status='complex'`／ 'Ace of Spades'(7) 'King of Spades'(2) 先頭 origin を `manual`／ antolakii(59) `aliases=['BVEP','Black Velvet Eastern Panama']`／ debile(34) 本文に「原記載では debilis」／ forgetii × 'Titanium'(55) はオーナーの播種日・作出者待ち／ `cultivar_images` の旧名3件を現名へ、`Anthurium テスト` 削除。
...
- **撤去（09-05）**: 画像 Real/Fake 投票、お気に入り（ナビ・ボタン・ページ）、トップの道具棚、トップの実生ノート区画、AdSense 枠、YouTube チャンネル検索（`searchYouTube`・`YOUTUBE_API_KEY`）、ユーザー名検索、i18n 死にキー、`trial-reminder` 関数、Amazon リンク、共有メニューの Facebook/LINE、`images/hero-mist.jpg`。**Platycerium 18件は `docs/data/platycerium-2026-09.csv` に退避のうえ削除**（`genera.id=4` も）。テーブル（`favorites` `image_votes`）は残す（可逆）。
- **残す**: 由来記録の投票（文字化「正確 / 疑問」、colophon・JSON-LD の「投票で検証」文言は削除し「出典の階層（S〜D）から算出」に統一。12月末に累計10票未満なら撤去）、今日の一葉、年表、索引、関連品種、用語自動リンク、使い方ガイド（投票・お気に入りの記述削除）。
...
| T26 | **検証ワークフロー**: 列3本・RPC・`cast_origin_vote` の固定・標本ラベル「検証状況」行・台帳 ✓・admin「検証 / 再確認キュー」 + 投票の文字化「正確 / 疑問」 | Claude Code | 9/20 | 由来の正確性を印にする |
| T27 | **詳細ページの読み順 + `.sheet` 部品 + Tier/バッジ統合 + 投稿フォーム整理**（design §1 C9〜C15, §2）→ プレビュー4パターン（写真あり / 写真なし原種 / AI のみ trust 23 / 調査中） | Claude Code → オーナー確認 | 9/24 | 詳細ページを製品に |
| T28 | **標本ラベル印刷**（print.css・`buildLabelHtml`・dialog・GA・i18n）→ iPhone Safari 印刷確認 | Claude Code → オーナー確認 | 9/27 | 会員特典・SNS 露出 |
| T29 | 今日の一葉 OGP 画像 + シェア文コピー、投稿完了画面のシェア、実生 OGP に交配式、人物索引「日本のブリーダー」区画 | Claude Code | 9/30 | Instagram 素材 |
| T30 | **料金 v2（Stripe 判定翌日）**: 1プラン化 会員 500/5,000、translations・料金・特商法・規約9条の2・モーダル・OGP・静的スタブ再生成、敗者側の決済関数・分岐削除、特典「先行閲覧」「広告非表示」「支援者」文言削除、ラベル印刷の門を `window._member` に | Claude Code | 判定 +1日 | 売上化 |
...
## 7. オーナー待ちタスク（手順つき、各 ≤2時間）

1. **Stripe ステータス確認**（0.5h、9/8）: ダッシュボードで審査状況を見る。変化なしならサポートへ問い合わせ（`docs/stripe-application.md` §5）。承認なら Price を2本作成（会員 月500円 `member_monthly` / 年5,000円 `member_annual`、税込）→ 公開鍵・秘密鍵・Price ID・Webhook 秘密を**チャットに貼らず** Supabase Secrets と `app-core.js` へ（画面共有で案内）。不承認なら PAY.JP 申請（特商法 URL `https://plantsstory.com/tokushoho/`、プランは会員2本のみ）。
2. **シード投稿 5件**（1.5h、9/12）: 投稿フォームから 'Dorayaki' → 'Red Vein Dark Phoenix' → 'Michelle' → 'Zara' → 'Titanium'。AI 調査後、交配式・作出者の明らかな誤りだけ直す。あわせて実生 forgetii × 'Titanium' の播種日・作出者を記録に追加。
3. **再調査リクエストの処理**（週2回・各15分、日・水）: 管理画面「再調査リクエスト」タブで承認/却下。承認後の結果は引用 URL を1件ずつ開き、正しければ「検証済にする」。会員は48時間以内。
4. **写真**（1.5h、9/21）: clarinervium・regale・luxurians・moronense・nutibarense・sagittatum のうち手元の株 + 交配種 2〜3（葉1枚正面・無地背景・長辺 2000px 以上）。`images/anthurium.png` と `images/og-default.png` の出所（自作か AI か）を一言で（未回答）。
5. **プレビュー確認**（各15分、iPhone 実機）: T21（9/9）、T27（9/24 の4パターン）、T28（印刷は共有シート → プリント）。「OK」か気になる点の箇条書きで返信。
6. **テストデータ・Platycerium 削除**（0.25h、9/10）: T36 の手順どおり管理画面/SQL で実行（CSV は `docs/data/` に保存済みであることを確認してから）。
7. **SNS**（週30分）: 週3回「今日の一葉」を投稿（T29 完成後はコピペ）。9/14 までに @hare_anthurium へ人物ページ完成の連絡、知人ブリーダー2名に「系統図に載せませんか」。
8. **10月の営業**（1h）: Claude Code が用意する DM 文面で国内アロイド専門店3店へ「取扱店枠（初月無料）」。
9. **楽天アフィリ**（20分、10月末）: 楽天カード or 楽天銀行と ID 連携 → 温湿度計・加湿器・LED のリンクを発行し Admin から登録。Yahoo!(VC) の既存リンク有効性を確認。
...
- **非公開の実生**（オーナー要望）:  列、所有者だけに見える RLS、RPC の明示フィルタ、投稿フォームのチェックボックス、詳細ページの切替、静的生成・sitemap・OGP からの除外。無料枠5件には非公開もカウント。
- **AI再調査リクエスト**: 登録時1回は無料、以後は無料の承認制（、管理画面で承認/却下）。由来調査の有料販売は取り下げ。
