...
| AI 調査コスト（09-05 決定） | 1回 約15〜20円（Web検索が大半）。歯止め: 検索8回/回、一般ユーザー 5回/日・同名3回/日、30日以内に調査済みなら一般は再調査不可、サイト全体 30回/日（Secrets `RESEARCH_DAILY_CAP`、到達時にメール通知）。無料登録1件≒20円の持ち出しは「収録が増えること自体が価値」として許容。月100件登録でも約2,000円 | 実装済 |
