...
(function (root, factory) {
  ...
  var AUTHORITY = /(^|\.)(ipni\.org|powo\.science\.kew\.org|gbif\.org|tropicos\.org|biodiversitylibrary\.org)\//i;
  var SIZE = /\d+(?:[.,]\d+)?\s?(?:cm|mm|m)\b/;
  ...
  function urlsOf(o) {
    if (!o) return [];
    var out = (o.sources || []).map(function (s) { return s && (s.url || s.text); });
    out = out.concat(((o.structured && o.structured.citation_links) || []).map(function (l) { return l && l.url; }));
    out.push(o.source_url);
  ...
  function qualifierOf(rec) {
    ...
    if (q) return q;
    var m = clean(rec && rec.cultivar_name).match(/\b(sp|aff|cf|ssp|subsp|var)\.(?:\s|$)/);
    return m ? m[1] : '';
  }
  function textLen(o) {
  ...
  function isIndividual(rec) {
    ...
    return tags.indexOf('individual') !== -1;
  }
  ...
  function gate(rec) {
    ...
    var authoritySource = os.some(function (x) { return urlsOf(x).some(function (u) { return AUTHORITY.test(u); }); });
    var humanLongest = Math.max.apply(null, os.map(function (x) { return (x.author && x.author.isAI) ? 0 : textLen(x); }));
    var sized = os.some(function (x) { return SIZE.test(clean(x.body) + ' ' + clean(x.structured && x.structured.notes)); });
